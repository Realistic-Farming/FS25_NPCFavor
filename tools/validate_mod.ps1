<#
.SYNOPSIS
    FS25_NPCFavor Mod Validator (PowerShell version)
...
Write-Header "FS25_NPCFavor Mod Validator"
...
foreach ($file in $LuaFiles) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        ...
        if (-not $isComment -and $line -match '\bprint\s*\(' -and $line -notmatch 'NPCFavor\.log' -and $RelPath -notmatch 'NPCFavorCore\.lua') {
            Add-Warning -File $RelPath -Category "DEBUG_CODE" -Line $lineNum -Message "Raw print() - use NPC Favor.log* instead"
        }
    }
}
...
# Only flag npc_* keys as missing (base game keys are expected to be in game's i18n)
foreach ($file in $LuaFiles) {
    $RelPath = $file.FullName.Substring($ModPath.Length + 1)
    $content = Get-Content $file.FullName -Raw -ErrorAction SilentlyContinue
    if (-not $content) { continue }
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        ...
        foreach ($match in $keyMatches) {
            ...
            # Only flag npc_* keys as errors (base game keys are assumed to exist)
            # Skip keys ending with _ (dynamic key construction like "npc_fluid_" .. fluidType)
            if ($key.StartsWith('npc_') -and -not $key.EndsWith('_') -and $key -notin $TransKeys) {
