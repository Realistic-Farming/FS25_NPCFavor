<#
.SYNOPSIS
    Extracts NPCFavor-related errors and warnings from FS25 game log.

.DESCRIPTION
    Parses the Farming Simulator 25 log.txt file and extracts all entries
    related to the NPCFavor mod, categorizing them by severity and type.
...
Write-Header "NPCFavor Log Analyzer"
...
# Patterns to match
$patterns = @{
    # NPCFavor specific patterns
    NPCFavorLine = 'NPC Favor|FS25_NPCFavor'
    ModLoaded = 'v(\d+\.\d+\.\d+)'
...
foreach ($i in 0..($lines.Count - 1)) {
    ...
    # Check if line is NPCFavor related
    if ($line -match $patterns.NPCFavorLine) {
        $stats.NPCFavorLines++
    ...
    if ($line -match $patterns.XmlParse -and $line -match 'NPCFavor') {
        $stats.XmlErrors += @{
            Line = $i + 1
            Text = $line.Trim()
            Type = "XmlError"
...
$report += "NPCFavor-Related Lines: $($stats.NPCFavorLines)"
